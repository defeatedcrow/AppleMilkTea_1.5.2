...
public class ItemPrincessClam extends Item {
					...
					if (target != null)
					{
						...
						if (flag)
						{
							...
							if (connected && thisWorld && sameDim)
			                {
								...
								thisPlayer.fallDistance = 0.0F;
								world.playSoundAtEntity(thisPlayer, "applemilk:suzu", 1.0F, 1.2F);
								thisPlayer.addChatMessage("Succeeded to warp near the registered player : " + target.getEntityName());
								return itemstack;
			                }
							else if (!sameDim)
							{
								thisPlayer.addChatMessage("Fail to warp near the registered player : " + target.getEntityName() + " is not alive in this dimention.");
							}
							else
							{
						...
						{
							thisPlayer.addChatMessage("Fail to warp near the registered player : " + "Fail to get position of around " + target.getEntityName() + ".");
						}
	...
	//プレイヤーに使った（マルチのみ）
//	@Override
//    public boolean itemInteractionForEntity(ItemStack itemstack, EntityPlayer player, EntityLiving entity)
//    {
...
//    				target.addChatMessage(player.getEntityName() + " registered you to the Raden Charm");
//        		}
//        		
//				return true;
//        	}
